...
RATE = 44100
CHANNELS = 1
OUTPUT_RAW_WAV_FILE = './streams/stream_raw_' + TIMESTAMP + '.wav'
...
def write_audio_file():
    global RAWAUDIO
    global REDUCEDAUDIO
    print('\nwriting audio file ' + OUTPUT_RAW_WAV_FILE)
    RAWAUDIO = np.array(RAWAUDIO, dtype=np.int16)
    REDUCEDAUDIO = np.array(REDUCEDAUDIO, dtype=np.int16)
    p.terminate()

    wf = wave.open(OUTPUT_RAW_WAV_FILE, 'wb')
    wf.setnchannels(CHANNELS)
    wf.setsampwidth(p.get_sample_size(pyaudio.paInt16))
    wf.setframerate(RATE)
    wf.writeframes(b''.join(RAWAUDIO))
    wf.close()

    wf = wave.open(OUTPUT_REDUCED_WAV_FILE, 'wb')
    wf.setnchannels(CHANNELS)
    wf.setsampwidth(p.get_sample_size(pyaudio.paInt16))
    wf.setframerate(RATE)
    wf.writeframes(b''.join(REDUCEDAUDIO))
    wf.close()
...
def noise_reduce(second):
    ...
    REDUCEDAUDIO.append(reduced_second)
    process_second(reduced_second)


stream = p.open(format=pyaudio.paInt16, channels=CHANNELS, rate=RATE, input=True, input_device_index=6, frames_per_buffer=CHUNK)
